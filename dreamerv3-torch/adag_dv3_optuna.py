...
def save_study_progress(study, save_dir):
    ...
    df.to_csv(os.path.join(save_dir, 'optuna_results_2D_flat_latent.csv'), index=False)
    joblib.dump(study, os.path.join(save_dir, 'optuna_study_2D_flat_latent.pkl'))
...
if __name__ == "__main__":
    ...
    study_file = os.path.join(directory, 'optuna_study_2D_flat_latent.pkl')
    if os.path.exists(study_file):
        print("Study file found. Resuming the study.")
        study = joblib.load(os.path.join(directory, 'optuna_study_2D_flat_latent.pkl'))
    else:
