...
def train_model(lr, batch_size, z_size, steps):
    ...
    model = AutoEncoder(
        encoder=EncoderConv32(x_shape=data_x_shape, z_size=z_size, z_multiplier=2),
        decoder=DecoderConv32(x_shape=data_x_shape, z_size=z_size),
    )
...
def get_gif():
    ...
    dataset = DisentDataset(dataset=data, sampler=RlSampler(), transform=ToImgTensorF32())
    dataloader = DataLoader(dataset=dataset, batch_size=225, shuffle=True, num_workers=0)
    batch = next(iter(dataloader))
