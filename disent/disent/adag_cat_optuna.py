...
def save_study_progress(study, save_dir):
    ...
    df.to_csv(os.path.join(save_dir, 'optuna_results_cat_square.csv'), index=False)
    joblib.dump(study, os.path.join(save_dir, 'optuna_study_cat_square.pkl'))
...
def objective(trial):
    ...
    latent_size=trial.suggest_int('latent_size',4,32)
    batchsize=trial.suggest_int('batch_size',4,64)
    num_classes=latent_size=trial.suggest_int('num_classes',4,32)
    ...
    model = AutoEncoder(
        encoder=EncoderConv64Categorical(x_shape=data.x_shape, z_size=latent_size, z_multiplier=1, n_classes=num_classes, latent_distribution='categorical' ),
        decoder=DecoderConv64Categorical(base_decoder=DecoderConv64(x_shape=data.x_shape, z_size=latent_size*num_classes ), z_size=latent_size,x_shape=data.x_shape,n_classes=num_classes ),
    )
...
if __name__ == "__main__":
    ...
    study_file = os.path.join(directory, 'optuna_study_cat_square.pkl')
    if os.path.exists(study_file):
        print("Study file found. Resuming the study.")
        study = joblib.load(os.path.join(directory, 'optuna_study_cat_square.pkl'))
    else:
